...
async function fetchMessages() {
  ...
  try {
    ...
    if (messages.length === 0) {
      listContainer.innerHTML = "<p>Aucun message dans la boîte de réception.</p>";
      return;
    }
...
async function fetchAdminProjects() {
  ...
  try {
    ...
    localProjectsArray.forEach((proj) => {
      ...
      const safeTitle = (proj.title || "Projet sans titre").replace(/'/g, "\\'");
...
window.prepareEditProject = function (projectId) {
  ...
  if (project.type && typeof project.type === "object") {
    document.getElementById("proj-type").value = project.type.Name || project.type.Label || "";
  } else {
  ...
  document.getElementById("form-project-title").innerText = `✏️ Modifier : ${project.title || ""}`;
  document.getElementById("btn-project-submit").innerText = "Enregistrer les modifications";
...
window.resetProjectForm = function () {
  ...
  document.getElementById("form-project-title").innerText = "Ajouter un Nouveau Projet";
...
document.getElementById("add-project-form")?.addEventListener("submit", async (e) => {
  ...
  formData.append("Github_Link", document.getElementById("proj-link1").value || "");
  formData.append("Github_Link_2", document.getElementById("proj-link2").value || "");
  formData.append("TypeName", document.getElementById("proj-type").value);
  formData.append("languages", document.getElementById("proj-languages").value);
...
window.deleteProjectAction = async function (projectId, projectTitle) {
  const confirmDelete = confirm(`⚠️ Êtes-vous sûr de vouloir supprimer "${projectTitle}" ?`);
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(`❌ Erreur : ${result.message || "Impossible de supprimer le projet"}`);
    }
  } catch (error) {
